...
from mmpose.models.utils.tta import flip_vector
...
class RTMCCHead2(BaseHead):
    ...
    def __init__(
        ...
        self.cls = nn.Linear(gau_cfg['hidden_dims'], 5, bias=False)
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        Returns:
            pred (Tensor): [mean_x, mean_y, sigma_x, sigma_y, rho]
        """
        feats = feats[-1]
    ...
    def predict(
        ...
        if test_cfg.get('flip_test', False):
            ...
            _batch_pred = self.forward(_feats)

            _batch_pred_flip = self.forward(_feats_flip)
            raise NotImplementedError("flip_indices is not defined")
            _batch_pred_flip = flip_vector(
                _batch_pred_flip,
                flip_indices=flip_indices)

            batch_pred = (_batch_pred + _batch_pred_flip) * 0.5
        else:
            batch_pred = self.forward(feats)

        preds = self.decode(batch_pred)
